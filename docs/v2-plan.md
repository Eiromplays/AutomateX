...
**v2.9 "convergence" (in progress — the big pre-v3 release).** Bundles matrix.onMessage (done) with: scheduled workflows (done), connection types (done), SignalR workspace groups (done), plugins-page tabs (done), refresh-token session renewal (done), and **conditionals/gates** (done) (the deterministic branching primitive — `filter`/`gate` step that halts the workflow unless a template expr is truthy; unlocks "check-and-act-if" without AI). Each ships as its own commit; the release is tagged once when the bundle lands. Items below move up into this release as built.
...
**Refresh-token session renewal (v2.9):** delivered. OIDC now adds the `offline_access` scope and `SaveTokens=true`, so the access token + its `expires_at` + the refresh token ride in the auth cookie. A cookie `OnValidatePrincipal` handler (`CookieTokenRefresher`) checks expiry on each request and, within a 2-minute leeway, trades the refresh token for a fresh set at the provider's `token_endpoint` — then `ShouldRenew=true` re-issues the cookie. A failed refresh (revoked/disabled user, rotated-away token) `RejectPrincipal()` + signs out, so **cookie liveness now tracks the IdP** instead of sliding blindly for 8h; no visible redirect bounce. Decision/parse/apply logic is pure and unit-tested (`OidcTokenRefresh`); the HTTP exchange is behind `IOidcTokenClient` so the reject-vs-renew flow is tested with a stub, no live IdP (`RefreshTokenTests`). Provider-agnostic by design — a lean `OnValidatePrincipal` over any `Authority`, not `Microsoft.Identity.Web` (which would couple us to Entra). The earlier "no refresh needed" take was too narrow: it answered "call APIs as the user?" (no), not "stay logged in revocably without a redirect?" (yes). Supersedes the "Auth session polish (NOT refresh tokens)" backlog note below.

**Session persistence fix (v2.9.1):** dogfooding surfaced that a restart bounced you to login — unrelated to refresh tokens. The auth cookie is encrypted by the ASP.NET **Data Protection** key ring, which was unconfigured = ephemeral (regenerated every restart in a container, invalidating all cookies). Refresh tokens can't help here: they refresh the access token *inside* a still-valid cookie, not a cookie that's become undecryptable. Fix: `AutomateXDbContext` implements `IDataProtectionKeyContext` and `AddDataProtection().PersistKeysToDbContext<AutomateXDbContext>().SetApplicationName("AutomateX")` — the key ring lives in the Postgres we already run, surviving restarts/recreation (needs one migration, `AddDataProtectionKeys`). The three lifetimes, clarified: **access token** (IdP, ~1h, lives in the cookie, refreshed near expiry) < **auth cookie / session** (`ExpireTimeSpan`, sliding, our knob) ≤ **refresh token** (IdP lifetime + revocation, the true ceiling). The cookie is meant to *outlast* the access token — making the access-token lifetime the session length would defeat refresh entirely. Session length is now `Auth__SessionLifetime` (TimeSpan, default 14d sliding = framework standard; non-positive guarded back to default — `SessionLifetimeTests`). This is the DP-key persistence that BFF stacks (Duende et al.) configure for you.
