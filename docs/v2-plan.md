...
- **Connection-reference helper** (worthwhile near-term UI win): step-config string fields hand-type `{{connections.matrix.accessToken}}` today — casing/typo-prone. Add an "insert connection reference" picker (lists existing connections + their keys, inserts the exact token) and a small 🔗 chip when a field value already is a connection reference. Generic over all string inputs since any can reference a connection.
